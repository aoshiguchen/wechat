...
(function($) {
	...
	$.msgbox = function(opts) {
		...
		opts.onConfrim =  opts.onConfrim || null;
		opts.anim = opts.anim || 0;
		opts.location = opts.location || $('body');
		this.opts = opts;
		this._hook_options();
		...
		this.background = $('<div />').css({
			...
			'left': 0,
			'display': 'none'
		}).appendTo(bodyObj).addClass('jMsgbox-background');
		...
		}).appendTo(bodyObj).addClass('jMsgbox-mainWrap');
		...
		this.closeWrap = $('<div />').css({  // it is a, not div
			'text-align': 'right'
		}).append('<a href="javascript:;">X</a>').appendTo(this.headWrap).find('a').click(function() {
			_this.close();
	...
	$.msgbox.prototype = {
		...
		show: function() {
			...
			//默认选中的焦点
			var buttonArray = $(this.mainWrap).find('input[type="button"]');
			$(buttonArray[0]).attr('isFocus','yes').focus();
			
			$(this.mainWrap).on('click',function(){
				$(buttonArray[0]).attr('isFocus','yes').focus();
			});
			
			//绑定tab键盘
			$(this.mainWrap).keydown(function(event) {
				if (event.which == 9) {
					event.preventDefault();
					if(buttonArray && buttonArray.length>1){
						$(buttonArray).each(function(index,element){
							var isFocus=$(element).attr('isFocus');
							if(isFocus && isFocus == 'yes'){//说明需要切换
								$(element).attr('isFocus','no');
								if(buttonArray[index+1]){
									$(buttonArray[index+1]).attr('isFocus','yes').focus();
								}else{
									$(buttonArray[0]).attr('isFocus','yes').focus();
								}
		...
		domready: function(){
			...
			this.extend('show', function(self){
				...
				switch(this.opts.anim){
					...
					case 3:
						_this.mainWrap.show().css({
							left:($(window).width() - _this.mainWrap.width()-5) + 'px',
...
/*				var interval = setInterval(function(){ // wait until the image loaded
				...
				/*_this.titleWrap.width(_this.opts.width -50);*/
		...
		domready: function(){
			...
			switch(this.opts.type){
				case 'alert':
					...
					var closebutton = $('<input type="button" value="确定" />');
					$(closebutton).on('click',function(){
						_this.close();	
					});
					var center = $('<div />').addClass('jMsgbox-footer').append(closebutton);
					
					var mainWarpObj = this.mainWrap;
					
					$(mainWarpObj).children('div:eq(1)').children('div').removeClass().addClass('warning');
				...
				case 'confirm':
					...
					var center = $('<div />').addClass('jMsgbox-footer').append(nobutton).append(yesbutton);
					
					var mainWarpObj = this.mainWrap;
					
					$(mainWarpObj).children('div:eq(1)').children('div').removeClass().addClass('prompt');
					...
					break;
				case 'customContent':
					var buttonarray=this.opts.buttonarray;
					_this.contentWrapWrap.height(parseInt(_this.contentWrapWrap.css('height'))-40);
					
					var center = $('<center />').css('padding','8px');
					$(buttonarray).each(function(index,element){
						$(center).append(element).append('&nbsp;');
					});
				...
				case 'ajax':
				case 'url':
					_this.contentWrap.text('加载中 请稍后 ...');
