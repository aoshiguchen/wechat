	...
	return $(this).each(function(){
		...
		tree
			...
			.bind('collapse',function(event){
				...
				target.next().slideUp(o.speed, function(){
		...
		tree	
			...
			.click(function(event){
				...
				if( target.is('a.tree-parent') ){
				    if(target.parent('li').parent('ul').is(tree) && o.only){
				        tree.find('li[aria-expanded="true"]>a').trigger('collapse');
				    }
				    target.trigger('toggle');
					target.eq(0).focus();
					tree.find('[class=tree-noleaf]').attr('class','tree-noleaf tree-parent-collapsed');
					//return click event false because it's a tree node (folder)
					return false;
				}
				if( target.is('a.tree-noleaf') ){
				    tree.find('[class=tree-parent]').trigger('collapse');
				    tree.find('[class=tree-noleaf]').attr('class','tree-noleaf tree-parent-collapsed');
				    target.attr('class','tree-noleaf');
                    //return click event true because it's NOT a tree node (folder)
                    return true;
                }
			})
