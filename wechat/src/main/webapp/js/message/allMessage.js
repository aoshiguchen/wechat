 $(function(){
	initallMessage("init",1,10);
	window.setInterval(makeNewMessage,30000);
});
 

function initallMessage(inits,pages,rows){
	...
	$.ajax({
		...
		success:function(data){
			...
			for(var i = 0;i<data.message.length;i++){
				...
				if(messReply!=null){
					for(var s=0;s<messReply.length;s++){
						alert(messReply[s].content);
					}
				}*/
				contents+="<tr class='list-info' >";
				contents+=" <td class='center'></td>";
				contents+="<td><div class='pop'  onmouseover='showDetail(1,"+i+")' onmouseout='showDetail(2,"+i+")' >";
				contents+="<table class='p-info'>";
				contents+="<tr><td width='30%' rowspan='2'><img src='"+(data.message[i].headimgurl==null?"":data.message[i].headimgurl)+"' width='40' height='40' class='img1'id='shows"+i+"' ></td>";
				contents+=" <td width='70%'></td></tr>";
				contents+="<tr><td>"+(data.message[i].nickname==null?"——":data.message[i].nickname)+"</td></tr>";
				contents+="</table>";
				...
				contents+="<div class='line float-p'> <span class='vm nickName'>"+(data.message[i].nickname==null?"——":data.message[i].nickname)+"</span> <span class='man_blue'></span> </div>";
				...
				contents+="<span>"+(data.message[i].province==null?"":data.message[i].province)+(data.message[i].city==null?"":data.message[i].city)+"</span></div></div>";
				...
				if(data.message[i].content!=null&&data.message[i].content.length>10){
					contents+="<td>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp"+data.message[i].content.substr(0,10)+"......</td>";
				}else{
					contents+="<td>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp"+(data.message[i].content==null?"————":data.message[i].content)+"</td>";
				}
				...
				contents+="<td><span class='blue2'>"+status+"</span></td>";
				
				var dealUser = data.message[i].dealUserName == null? "":data.message[i].dealUserName;
				
				contents+="<td><span>"+dealUser+"</span></td>";
				
				contents+=" <td><div class='op-icons'> <a  class='icon01' name='"+data.message[i].messageId+"' id='"+data.message[i].fansId+"' title='"+data.message[i].sessionId+"' onClick='showInfo(this,"+data.message[i].status+")'></a></div></td></tr>";
			...
			if(data.page.currentPage==1){
				...
				makepage+="<span class='btns pre-yes' onclick='initallMessage("+'""'+","+(data.page.currentPage-1)+","+data.page.pageSize+")'></span>";
			}
			
			if(data.page.currentPage==data.page.totalPages){
				makepage+="<span class='btns next-no'></span>";
			}else{
				makepage+="<span class='btns next-yes' onclick='initallMessage("+'""'+","+(data.page.currentPage+1)+","+data.page.pageSize+")'></span>";
			}
			
			makepage+="</div>";
			if(inits=="init"){
				$("#totalRows").val(data.page.totalRows);
			}
		...
		$.ajax({
			...
			success:function(data){
				if(data=="0"){
					...
					initallMessage('',$("#currentPage").val(),$("#pageSize").val());
